...
void CalibrationDialog::setupUI() {
    ...
    // Группа географических координат
...
QVector<QString> CalibrationDialog::findMatchingFiles() {
    ...
    for (const QFileInfo& xmlFileInfo : xmlFiles) {
        ...
        // Парсинг XML для извлечения метаданных
        while (!xml.atEnd()) {
            xml.readNext();
        ...
        if (imagingDate.isEmpty()) {
            resultsText->append(QString("⚠ %1: нет даты съемки").arg(xmlFileInfo.fileName()));
            continue;
        }
        ...
        if (imageDateTime.isValid()) {
            ...
            resultsText->append(QString("⚠ %1: некорректная дата - %2 %3").arg(xmlFileInfo.fileName()).arg(imagingDate).arg(imagingTime));
            continue;
        }
        ...
        if (hasCoordinates && latitudes.size() == longitudes.size()) {
            ...
            resultsText->append(QString("⚠ %1: нет координат в метаданных").arg(xmlFileInfo.fileName()));
        }
    }
...
CalibrationDialog::CalibrationParams CalibrationDialog::getParameters() const {
    return params;
}
